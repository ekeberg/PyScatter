...
class SimplePDB:
    def __init__(self, filename, code):
        ...
        self.struct = self._parser.get_structure(code, filename)
...
class RectangularDetector:
    ...
    def solid_angle(self):
        sa = (self._omega(self.x+self.pixel_size[0]/2, self.y+self.pixel_size[1]/2) +
              self._omega(self.x-self.pixel_size[0]/2, self.y-self.pixel_size[1]/2) -
              self._omega(self.x+self.pixel_size[0]/2, self.y-self.pixel_size[1]/2) -
              self._omega(self.x-self.pixel_size[0]/2, self.y+self.pixel_size[1]/2)) / 4
        return sa
...
class Source:
    def __init__(self, photon_energy, beam_energy, diameter, polarization_angle=None):
        ...
        self.area = numpy.pi*(diameter/2)**2
        self.number_of_photons = conversions.J_to_ev(beam_energy)
        self.intensity = self.number_of_photons / self.area


def calculate_diffraction(pdb, detector, photon_energy, rotation=(1, 0, 0, 0)):
    ...
    diff = numpy.zeros(det.shape, dtype="complex128")
    for coord, element, occupancy in zip(pdb.coords, pdb.elements, pdb.occupancy):
        diff += (sf.precalculated[element] * occupancy *
                 numpy.exp(2j * numpy.pi * (coord[:, numpy.newaxis, numpy.newaxis] * S).sum(axis=0)))

    return diff


def calculate_pattern(pdb, detector, source, rotation=(1, 0, 0, 0)):
    diff = calculate_diffraction(pdb, detector, source.photon_energy, rotation)
    pattern  = abs(diff)**2
    pattern *= detector.solid_angle()
    pattern *= cross_section(det, source.photon_energy, source.polarization_angle)
    pattern *= src.intensity
    return pattern


pdb = SimplePDB(pdbfile, "foo")
src = Source(1200, 4e-3, 5e-6)
det = RectangularDetector((128, 128), 8*75e-6, 0.15)
rotation = numpy.array([ 0.20109424, -0.86721112,  0.        ,  0.45552825])

# pattern = calculate_pattern(pdb, det, src)

# diff = calculate_diffraction(pdb, detector, src.photon_energy)
# pattern *= det.solid_angle()
# pattern *= cross_section(det, 1200, 0)
# pattern *= src.intensity
